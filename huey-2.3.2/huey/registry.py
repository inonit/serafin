...
class Registry(object):
    ...
    def string_to_task(self, task_str):
        if 'init_session' in task_str:
            task_str='system.tasks.init_session'
        elif 'transition' in task_str:
            task_str='system.tasks.transition'
    ...
    def create_task(self, message):
        ...
        if not isinstance(message, Message) and isinstance(message, tuple):
            if len(message) == 7:
                tid, name, eta, retries, retry_delay, (
                    args, kwargs), oc = message
            elif len(message) == 6:
                tid, name, eta, retries, retry_delay, (args, kwargs) = message
                oc = None
